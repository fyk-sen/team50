...
CONFUSION_MATRIX_PATH = os.path.join(DATA_PATH, "confusion_matrix.csv")
PREDICTIONS_PATH = os.path.join(DATA_PATH, "predictions.csv")

def run_inference():
    """Run model inference and generate metrics."""
    if not os.path.exists(MODEL_PATH):
        return {"error": "Model not found. Train the model first."}, 404
    if not os.path.exists(X_TEST_PATH) or not os.path.exists(Y_TEST_PATH):
        return {"error": "Test data files not found. Ensure test data and labels are available."}, 404
    ...
    # Convert predictions: True → 1, False → 0
    if isinstance(Y_pred[0], (bool, np.bool_)):
    ...
    return {"message": "Inference completed successfully!"}, 200

@app.route("/process_inference", methods=["GET"])
def process_inference():
    """Consolidated endpoint for evaluation, metrics, and predictions."""
    ...
    if status == 200:
        ...
        return jsonify({
            ...
            "classification_report": metrics_df.to_dict(),
